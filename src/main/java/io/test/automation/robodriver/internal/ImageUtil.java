...
public class ImageUtil {
	...
	public Rectangle findRectangle(RoboScreen screen, RoboImage img) {
		...
		return findRectangle(source, img);
	}

	private Rectangle findRectangle(BufferedImage source, RoboImage img) {
	...
	public Rectangle findRectangle(RoboScreen screen, RoboScreenRectangle parentRectangle, RoboImage img) {
		if (parentRectangle == null) {
			return findRectangle(screen, img);
		} else {
			BufferedImage source = screen.getScreenCapture(parentRectangle);
			Rectangle rectangle = findRectangle(source, img);
	...
	private void checkImages(BufferedImage source, BufferedImage tofind) {
		if (tofind.getWidth() > source.getWidth()) {
			throw new ImageUtilException("The find-image width %d exceeds the screen width %d.", tofind.getWidth(),
					source.getWidth());
		}
		if (tofind.getHeight() > source.getHeight()) {
			throw new ImageUtilException("The find-image height %d exceeds the screen height %d.", tofind.getHeight(),
					source.getHeight());
		}
	}
	...
	public boolean equals(int color1, int color2, boolean includeAlpha) {
		return includeAlpha ? color1 == color2 : (color1 << 8) == (color2 << 8);
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < gds.length; i++) {
			...
			System.out.printf("Screen: id = '%s', width = %d, height = %d%n", gd.getIDstring(), dm.getWidth(),
					dm.getHeight());
			...
			for (int j = 0; j < cs.getNumComponents(); j++) {
				System.out.printf("                   component-%d: %s, min=%f, max=%f%n", j, cs.getName(j),
						cs.getMinValue(j), cs.getMaxValue(j));
			}
		}
	}
}
